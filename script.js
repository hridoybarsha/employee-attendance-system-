...
function animateValue(id, start, end, duration = 800, prefix = '') {
    ...
    const step = (timestamp) => {
        ...
        obj.textContent = prefix + currentValue.toLocaleString('en-IN');

        if (progress < 1) {
            window.requestAnimationFrame(step);
        } else {
            obj.textContent = prefix + Number(end).toLocaleString('en-IN');
        }
    };
    window.requestAnimationFrame(step);
}
...
function initLiveBackground() {
    ...
    const colorThemes = [
        { bg: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)", pColor: "96, 165, 250" }, // Blue
        { bg: "linear-gradient(135deg, #111827 0%, #1f2937 50%, #111827 100%)", pColor: "52, 211, 153" }, // Emerald Green
        { bg: "linear-gradient(135deg, #18181b 0%, #27272a 50%, #18181b 100%)", pColor: "168, 85, 247" }, // Purple
        { bg: "linear-gradient(135deg, #0f172a 0%, #172554 50%, #0f172a 100%)", pColor: "56, 189, 248" }  // Cyan Blue
    ];

    let currentThemeIndex = 0;

    // Cycle through themes every 8 seconds
    setInterval(() => {
...
async function loadAllData() {
    ...
    try {
        ...
        if (error.code === 'permission-denied') {
            alert("Firestore Error: Permission Denied!\n\nFirebase Console > Firestore Database > Rules-  'allow read, write: if true;'    ");
        }
    }
}
...
function updateDashboard() {
    ...
    animateValue("presentToday", 0, todayRecords.filter(a => a.status === "Full Day").length);
    animateValue("nightShiftToday", 0, todayRecords.filter(a => a.status === "Night Shift" || a.status === "Night Duty").length);
    ...
    const totalSalary = currentPayroll.reduce((sum, p) => sum + Number(p.netSalary || 0), 0);
    ...
    if (todayRecords.length === 0) {
        tbody.innerHTML = `<tr><td colspan="3" style="text-align:center;">No attendance recorded today</td></tr>`;
        return;
    }
...
function renderAttendanceCalendar() {
    ...
    let summary = {
        "Full Day": 0, "Night Shift": 0, "Half Day": 0, "Absent": 0, "Paid Leave": 0,
...
function calculateEmployeePayroll(employee, month) {
    ...
    const earnedSalary = (fullDay + nightShift + paidLeave + (halfDay * 0.5)) * perDay;
    const netSalary = earnedSalary;

    return {
        employeeId: employee.employeeId,
        month, basicSalary, fullDay, nightShift, halfDay, paidLeave, absent,
        bonus: 0, advance: 0, deduction: 0, earnedSalary, netSalary
    };
}
    ...
    payrollRows.forEach(row => {
        ...
        table.innerHTML += `
            <tr>
                ...
                <td>
                    <button class="action-btn edit-btn" onclick="savePayroll('${escapeHTML(row.employeeId)}', '${escapeHTML(row.month)}', ${row.basicSalary}, ${row.fullDay}, ${row.nightShift}, ${row.halfDay}, ${row.paidLeave}, ${row.absent}, ${row.netSalary})">Save</button>
                </td>
            </tr>`;
    });
...
window.savePayroll = async function(employeeId, month, basicSalary, fullDay, nightShift, halfDay, paidLeave, absent, calculatedNetSalary) {
    try {
        const existing = payrollData.find(p => p.employeeId === employeeId && p.month === month);

        const data = {
            employeeId, month, basicSalary, fullDay, nightShift, halfDay, paidLeave, absent,
...
function generateReport() {
    ...
    head.innerHTML = `<tr><th>Employee ID</th><th>Employee</th><th>Date</th><th>Status</th></tr>`;
    ...
    currentReportData.forEach(a => {
        ...
        body.innerHTML += `<tr><td>${escapeHTML(a.employeeId)}</td><td>${escapeHTML(employee?.name || "-")}</td><td>${escapeHTML(a.date)}</td><td>${escapeHTML(a.status)}</td></tr>`;
    });
}
