...
// 🎯 MONEY FORMATTER WITH MINUS SUPPORT
...
// 💵 ATTENDANCE PAGE MONEY SUMMARY UPDATE FUNCTION (FIXED ACCORDING TO YOUR EXACT LOGIC)
function updateAttendanceMoneySummary(employeeId, month) {
    if (!employeeId) {
        ...
        setText("attendanceCompanyDue", "Tk 0");
        return;
    }
    ...
    // ৩. কত টাকা পাবো (অর্জিত টাকা থেকে এডভান্স বিয়োগ -> ৪১৯ - ২০০ = ২১৯ টাকা)
    setText("attendanceNetDue", money(payroll.netSalary));
    
    // ৪. কোম্পানি কত পাবে (বেসিক ১৩,০০০ টাকার চেয়ে এডভান্স বেশি নিলে মাইনাস দেখাবে, কম নিলে Tk 0)
    setText("attendanceCompanyDue", money(payroll.companyDue));
}
...
/* ================= PAYROLL (ACCURATE CALCULATION FORMULA) ================= */
function calculateEmployeePayroll(employee, month) {
    ...
    // 🎯 ১. কত টাকা পাবো = অর্জিত টাকা থেকে নেওয়া এডভান্স বাদ (যেমন ৪১৯ - ২০০ = ২১৯ টাকা)
    const netSalary = Math.max(0, Math.round(totalCalculated - advanceTaken));

    // 🎯 ২. কোম্পানি কত পাবে
    // এডভান্স যদি বেসিক স্যালারির চেয়ে বেশি হয় (যেমন ১৪,০০০ টাকা), কেবল তখনই মাইনাস দিয়ে কোম্পানি পাওনা দেখাবে (-১,০০০)
    let companyDue = 0;
    if (advanceTaken > basicSalary) {
        companyDue = Math.round(basicSalary + bonus - deduction - advanceTaken); // যেমন: ১৩০০০ - ১৪০০০ = -১০০০ টাকা
    } else {
        companyDue = 0; // এডভান্স কম থাকলে কোম্পানি ০ টাকা পাবে
    }
