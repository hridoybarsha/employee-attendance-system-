...
function initializeFirebase() {
    try {
        ...
        // Safe Network Polling for local and unstable connection
        db.settings({
...
async function loadAllData() {
    ...
    try {
        setFirebaseStatus("connecting", "🟡 Loading...");

        const employeeSnap = await db.collection("employees").get();
        employees = employeeSnap.docs.map(doc => ({ firestoreId: doc.id, ...doc.data() }));

        const attendanceSnap = await db.collection("attendance").get();
        attendanceData = attendanceSnap.docs.map(doc => ({ firestoreId: doc.id, ...doc.data() }));

        const leaveSnap = await db.collection("leaves").get();
        leaveData = leaveSnap.docs.map(doc => ({ firestoreId: doc.id, ...doc.data() }));

        const payrollSnap = await db.collection("payroll").get();
        payrollData = payrollSnap.docs.map(doc => ({ firestoreId: doc.id, ...doc.data() }));

        try {
            const settingsDoc = await db.collection("settings").doc("office").get();
            if (settingsDoc.exists) {
                settings = { ...settings, ...settingsDoc.data() };
            }
        } catch (sErr) {
            console.warn("Settings doc not found, using default.", sErr);
        }

        setFirebaseStatus("connected", "🟢 Connected");
        refreshUI();
    } catch (error) {
        console.error("Firestore Load Error Detail:", error);
        setFirebaseStatus("error", "🔴 Firestore Error");

        if (error.code === 'permission-denied') {
            alert("Firestore Error: Permission Denied!\n\nFirebase Console > Firestore Database > Rules-এ দিয়ে 'allow read, write: if true;' টেস্ট মোড সেভ করুন।");
        }
    }
}
...
function updateDashboard() {
    ...
    setText("presentToday", todayRecords.filter(a => a.status === "Full Day").length);
    setText("halfDayToday", todayRecords.filter(a => a.status === "Half Day").length);
    setText("absentToday", todayRecords.filter(a => a.status === "Absent").length);
    setText("leaveToday", todayRecords.filter(a => a.status === "Paid Leave" || a.status === "Unpaid Leave").length);
    setText("lateToday", todayRecords.filter(a => Number(a.late || 0) > 0).length);
    setText("overtimeToday", todayRecords.reduce((sum, a) => sum + Number(a.overtime || 0), 0).toFixed(2));

    const currentMonth = monthString(new Date());
    const currentPayroll = payrollData.filter(p => p.month === currentMonth);

    setText("monthlyPayroll", money(currentPayroll.reduce((sum, p) => sum + Number(p.netSalary || 0), 0)));
    ...
    if (todayRecords.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">No attendance recorded today</td></tr>`;
        return;
    }
...
function renderEmployees() {
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="9" style="text-align:center;">No Employees Found</td></tr>`;
...
function renderAttendanceCalendar() {
    ...
    setText("calendarTitle", currentCalendarDate.toLocaleDateString("en-IN", { month: "long", year: "numeric" }));

    const firstDay = new Date(year, month, 1).getDay();
    const daysInMonth = new Date(year, month + 1, 0).getDate();

    let summary = {
        "Full Day": 0, "Half Day": 0, "Absent": 0, "Paid Leave": 0,
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        calendar.innerHTML += `
            <div class="calendar-day" onclick="openAttendanceForDate('${date}')">
...
window.openAttendanceForDate = function(date) {
    ...
    if ($("attendanceStatus")) $("attendanceStatus").value = record?.status || "Full Day";
...
function initAttendanceEvents() {
    ...
    form.addEventListener("submit", async event => {
        ...
        const checkIn = safeValue("checkInTime");
        const checkOut = safeValue("checkOutTime");
        const status = safeValue("attendanceStatus");

        const late = calculateLate(checkIn);
        const overtime = calculateOvertime(checkOut);
...
function calculateLate(checkIn) {
    ...
    const grace = Number(settings.gracePeriod || 0);
...
function calculateOvertime(checkOut) {
    ...
    return Number(Math.max(0, (actual - end) / 60).toFixed(2));
}
        ...
        try {
            for (let day = 1; day <= days; day++) {
                ...
                const data = {
                    ...
                    date,
                    status: "Full Day",
                    checkIn: settings.officeStartTime,
...
function calculateEmployeePayroll(employee, month) {
    ...
    const overtime = records.reduce((sum, a) => sum + Number(a.overtime || 0), 0);

    const basicSalary = Number(employee.salary || 0);
    const parts = month.split("-");
    const daysInMonth = new Date(Number(parts[0]), Number(parts[1]), 0).getDate();

    const perDay = basicSalary / daysInMonth;
    const earnedSalary = (fullDay + paidLeave + (halfDay * 0.5)) * perDay;
    const overtimeAmount = overtime * Number(settings.overtimeRate || 0);
    const netSalary = earnedSalary + overtimeAmount;

    return {
        employeeId: employee.employeeId,
        month, basicSalary, fullDay, halfDay, paidLeave, absent,
        overtime, bonus: 0, advance: 0, deduction: 0, overtimeAmount, earnedSalary, netSalary
    };
}
...
async function calculatePayroll() {
    ...
    payrollRows.forEach(row => {
        ...
        totalOvertime += Number(row.overtime);
        ...
        table.innerHTML += `
            <tr>
                ...
                <td>
                    <button class="action-btn edit-btn" onclick="savePayroll('${escapeHTML(row.employeeId)}', '${escapeHTML(row.month)}', ${row.basicSalary}, ${row.fullDay}, ${row.halfDay}, ${row.paidLeave}, ${row.absent}, ${row.overtime}, ${row.netSalary})">Save</button>
                </td>
            </tr>`;
    });
    ...
    setText("totalOvertimeHours", totalOvertime.toFixed(2) + " Hrs");
}

window.savePayroll = async function(employeeId, month, basicSalary, fullDay, halfDay, paidLeave, absent, overtime, calculatedNetSalary) {
    try {
        const existing = payrollData.find(p => p.employeeId === employeeId && p.month === month);

        const data = {
            employeeId, month, basicSalary, fullDay, halfDay, paidLeave, absent, overtime,
...
function initSalarySlip() {
    ...
    button.addEventListener("click", () => {
        ...
        setText("slipOvertime", Number(payroll.overtime || 0).toFixed(2) + " Hrs");
        setText("slipBasicSalary", money(payroll.basicSalary || employee.salary || 0));
...
function generateReport() {
    ...
    head.innerHTML = `<tr><th>Employee ID</th><th>Employee</th><th>Date</th><th>Status</th><th>Late</th><th>Overtime</th></tr>`;
    currentReportData = attendanceData.filter(a => String(a.date || "").startsWith(month));

    if (type === "late") currentReportData = currentReportData.filter(a => Number(a.late || 0) > 0);
    if (type === "overtime") currentReportData = currentReportData.filter(a => Number(a.overtime || 0) > 0);

    currentReportData.forEach(a => {
        const employee = employees.find(e => e.employeeId === a.employeeId);
        body.innerHTML += `<tr><td>${escapeHTML(a.employeeId)}</td><td>${escapeHTML(employee?.name || "-")}</td><td>${escapeHTML(a.date)}</td><td>${escapeHTML(a.status)}</td><td>${a.late || 0} Min</td><td>${a.overtime || 0} Hrs</td></tr>`;
    });
}
...
function loadSettingsForm() {
    if ($("officeStartTime")) $("officeStartTime").value = settings.officeStartTime;
    if ($("officeEndTime")) $("officeEndTime").value = settings.officeEndTime;
    if ($("gracePeriod")) $("gracePeriod").value = settings.gracePeriod;
    if ($("weeklyOff")) $("weeklyOff").value = settings.weeklyOff;
    if ($("overtimeRate")) $("overtimeRate").value = settings.overtimeRate;
}
...
function initSettings() {
    ...
    form.addEventListener("submit", async event => {
        ...
        settings = {
            officeStartTime: safeValue("officeStartTime"),
            officeEndTime: safeValue("officeEndTime"),
            gracePeriod: Number(safeValue("gracePeriod") || 0),
            weeklyOff: Number(safeValue("weeklyOff") || 0),
            overtimeRate: Number(safeValue("overtimeRate") || 0)
        };
