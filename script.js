...
/* ================= PAYROLL (ACCURATE DYNAMIC MONTH CALCULATION) ================= */
function calculateEmployeePayroll(employee, month) {
    ...
    //  DYNAMIC FIXED MONTHLY MODEL (28, 30, or 31 Days Based Calculation)
    const perDaySalary = basicSalary / daysInMonth;
    
    //       
    const deductionForAbsent = absent * perDaySalary;
    const deductionForHalfDay = halfDay * (perDaySalary * 0.5);

    //   
    const earnedSalary = basicSalary - (deductionForAbsent + deductionForHalfDay);
    const netSalary = Math.round(Math.max(0, earnedSalary));

    return {
        employeeId: employee.employeeId,
        month, basicSalary, fullDay, nightShift, halfDay, paidLeave, absent,
        bonus: 0, advance: 0, deduction: Math.round(deductionForAbsent + deductionForHalfDay), earnedSalary, netSalary
    };
}
