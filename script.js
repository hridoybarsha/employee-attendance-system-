...
//  ROUNDED MONEY FORMATTER (NO DECIMAL)
...
function updateDashboard() {
    ...
    animateValue("monthlyPayroll", 0, totalSalary, 1000, "Tk ");
...
function renderAttendanceCalendar() {
    ...
    //  UPDATE MONEY SUMMARY IN ATTENDANCE PAGE
    updateAttendanceMoneySummary(employeeId, currentMonthStr);
}

//  ATTENDANCE PAGE MONEY SUMMARY UPDATE FUNCTION
function updateAttendanceMoneySummary(employeeId, month) {
    if (!employeeId) {
        animateValue("attendanceEarnedSalary", 0, 0, 400, "Tk ");
        animateValue("attendanceAdvanceTaken", 0, 0, 400, "Tk ");
        animateValue("attendanceNetDue", 0, 0, 400, "Tk ");
        return;
    }
    ...
    animateValue("attendanceEarnedSalary", 0, payroll.earnedSalary, 600, "Tk ");
    animateValue("attendanceAdvanceTaken", 0, payroll.advance, 600, "Tk ");
    animateValue("attendanceNetDue", 0, payroll.netSalary, 600, "Tk ");
}
...
/* ================= PAYROLL (ACCURATE PRESENT-BASED CALCULATION) ================= */
function calculateEmployeePayroll(employee, month) {
    ...
    const totalWorkingDays = fullDay + nightShift + paidLeave + (halfDay * 0.5);
    ...
    const advance = Number(existing?.advance || 0);
    const deduction = Number(existing?.deduction || 0);

    const netSalary = Math.max(0, Math.round(earnedSalary + bonus - advance - deduction));

    return {
        employeeId: employee.employeeId,
        month, basicSalary, fullDay, nightShift, halfDay, paidLeave, absent,
        bonus, advance, deduction, earnedSalary, netSalary
    };
}
...
function generateReport() {
    ...
    head.innerHTML = `<tr><th>Employee ID</th><th>Employee</th><th>Date</th><th>Status</th><th>Pipe Type</th><th>Running Machine</th></tr>`;
    ...
    currentReportData.forEach(a => {
        ...
        body.innerHTML += `<tr><td>${escapeHTML(a.employeeId)}</td><td>${escapeHTML(employee?.name || "-")}</td><td>${escapeHTML(a.date)}</td><td>${escapeHTML(a.status)}</td><td>${escapeHTML(a.pipeType || "None")}</td><td>${escapeHTML(a.machine || "None")}</td></tr>`;
    });
}
