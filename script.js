...
function updateDashboard() {
    ...
    animateValue("presentToday", 0, todayRecords.filter(a => a.status === "Full Day" || a.status === "Full Day/Night Shift" || a.status === "Night/Full Day Shift" || a.status === "Paid Weekly Off").length);
    animateValue("nightShiftToday", 0, todayRecords.filter(a => a.status === "Night Shift" || a.status === "Night Duty" || a.status === "Full Day/Night Shift" || a.status === "Night/Full Day Shift").length);
...
/* ================= ATTENDANCE CALENDAR & SUMMARY (STRICTLY 1-30 DAYS COUNTING) ================= */
...
function statusEmoji(status) {
    const map = {
        ...
        "Night Shift": "🌙",
        "Night Duty": "🌙",
        "Full Day/Night Shift": "☀️🌙",
        "Night/Full Day Shift": "🌙☀️",
        "Half Day": "🟡",
        "Absent": "🔴",
        "Paid Leave": "🔵",
        "Paid Weekly Off": "☑️",
...
function renderAttendanceCalendar() {
    ...
    const daysInMonth = new Date(year, month + 1, 0).getDate(); // ক্যালেন্ডারে ৩১ দিন পর্যন্তই রেন্ডার হবে

    let summary = {
        "Full Day": 0, "Night Shift": 0, "Full Day/Night Shift": 0, "Night/Full Day Shift": 0, "Half Day": 0, "Absent": 0, "Paid Leave": 0,
        "Paid Weekly Off": 0, "Unpaid Leave": 0, "Holiday": 0, "Weekly Off": 0, "Not Set": 0
    };
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // সামারি এবং কাউন্টিংয়ের সময় শুধুমাত্র ১ থেকে ৩০ তারিখ পর্যন্ত কাউন্ট হবে (৩১ তারিখের ডেট সম্পূর্ণ বাদ থাকবে)
        if (employeeId && day <= 30) {
    ...
    setText("summaryNightFullDay", summary["Night/Full Day Shift"]);
    setText("summaryHalfDay", summary["Half Day"]);
    setText("summaryAbsent", summary["Absent"]); // এখন এটি একদম সঠিক ৭টি দেখাবে
    setText("summaryPaidLeave", summary["Paid Leave"]);
    
    if($("summaryPaidWeeklyOff")) setText("summaryPaidWeeklyOff", summary["Paid Weekly Off"]);
    ...
    // ১ থেকে ৩০ তারিখের ওপর ভিত্তি করে মোট ডিউটি গণনা
    let totalDuties = 0;
    for (let day = 1; day <= 30; day++) {
        ...
        let st = record ? record.status : (isWeeklyOff ? "Weekly Off" : "Not Set");
        if (st === "Night Duty") st = "Night Shift";

        if (st === "Full Day") totalDuties += 1;
        else if (st === "Night Shift") totalDuties += 1;
        else if (st === "Full Day/Night Shift") totalDuties += 2;
        else if (st === "Night/Full Day Shift") totalDuties += 2;
        else if (st === "Half Day") totalDuties += 0.5;
        else if (st === "Paid Leave") totalDuties += 1;
        else if (st === "Paid Weekly Off") totalDuties += 1;
    }

    setText("summaryTotalDuties", totalDuties + " Days");
...
/* ================= PAYROLL (STRICT 30 DAYS NORM - EXCLUDING 31st DAY FROM COUNT) ================= */
function calculateEmployeePayroll(employee, month) {
    // শুধুমাত্র ১ থেকে ৩০ তারিখ পর্যন্ত রেকর্ডগুলো ফিল্টার করা হচ্ছে (৩১ তারিখের ডেট সম্পূর্ণ বাদ)
    const records = attendanceData.filter(a => {
        if (!a.employeeId || !a.date || a.employeeId !== employee.employeeId || !String(a.date).startsWith(month)) return false;
        const dayPart = parseInt(String(a.date).split("-")[2], 10);
        return dayPart <= 30;
    });
    ...
    const nightFullDayShift = records.filter(a => a.status === "Night/Full Day Shift").length;
    const halfDay = records.filter(a => a.status === "Half Day").length;
    const paidLeave = records.filter(a => a.status === "Paid Leave").length;
    const paidWeeklyOff = records.filter(a => a.status === "Paid Weekly Off").length;
    ...
    // পার-ডে স্যালারি সবসময় কঠোরভাবে ফিক্সড ৩০ দিন দিয়েই হিসাব হবে
    const fixedDaysInMonth = 30;

    const totalWorkingDays = fullDay + nightShift + (fullDayNightShift * 2) + (nightFullDayShift * 2) + paidLeave + paidWeeklyOff + (halfDay * 0.5);
    
    // নিখুঁতভাবে ৩০ দিনের মাপে হিসাব
    const earnedSalary = Math.round((basicSalary / fixedDaysInMonth) * totalWorkingDays);
    ...
    return {
        ...
        month, basicSalary, fullDay, nightShift, fullDayNightShift, nightFullDayShift, halfDay, paidLeave, absent,
    ...
    payrollRows.forEach(row => {
        ...
        table.innerHTML += `
            <tr>
                ...
                <td>
                    <button class="action-btn edit-btn" onclick="savePayroll('${escapeHTML(row.employeeId)}', '${escapeHTML(row.month)}', ${row.basicSalary}, ${row.fullDay}, ${row.nightShift}, ${row.fullDayNightShift || 0}, ${row.nightFullDayShift || 0}, ${row.halfDay}, ${row.paidLeave}, ${row.absent}, ${row.netSalary})">Save</button>
                </td>
            </tr>`;
    });
...
window.savePayroll = async function(employeeId, month, basicSalary, fullDay, nightShift, fullDayNightShift, nightFullDayShift, halfDay, paidLeave, absent, calculatedNetSalary) {
    try {
        const existing = payrollData.find(p => p.employeeId === employeeId && p.month === month);

        const data = {
            employeeId, month, basicSalary, fullDay, nightShift, fullDayNightShift, nightFullDayShift, halfDay, paidLeave, absent,
...
function initSalarySlip() {
    ...
    button.addEventListener("click", () => {
        ...
        setText("slipNightFullDay", payroll.nightFullDayShift || 0);
