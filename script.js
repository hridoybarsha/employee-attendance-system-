...
function updateDashboard() {
    ...
    animateValue("presentToday", 0, todayRecords.filter(a => a.status === "Full Day" || a.status === "Full Day/Night Shift" || a.status === "Night/Full Day Shift" || a.status === "Weekly Off Paid").length);
...
function getStatusBgClass(status) {
    switch (status) {
        ...
        case "Weekly Off Paid": return "bg-paid-leave";
        case "Unpaid Leave": return "bg-unpaid-leave";
        case "Holiday": return "bg-holiday";
        case "Weekly Off Unpaid": return "bg-weekly-off";
        default: return "";
    }
}
...
function renderAttendanceCalendar() {
    ...
    let summary = {
        ...
        "Weekly Off Paid": 0, "Unpaid Leave": 0, "Holiday": 0, "Weekly Off Unpaid": 0, "Not Set": 0
    };
    ...
    if($("summaryPaidWeeklyOff")) setText("summaryPaidWeeklyOff", summary["Weekly Off Paid"]);
    
    setText("summaryUnpaidLeave", summary["Unpaid Leave"]);
    setText("summaryHoliday", summary["Holiday"]);
    setText("summaryWeeklyOff", summary["Weekly Off Unpaid"]);
    ...
    for (let day = 1; day <= 30; day++) {
        ...
        let st = record ? record.status : (isWeeklyOff ? "Weekly Off Unpaid" : "Not Set");
        ...
        else if (st === "Weekly Off Paid") totalDuties += 1;
    }
...
function calculateEmployeePayroll(employee, month) {
    const records = attendanceData.filter(a => {
        ...
        return dayPart <= 30; // ৩১ তারিখের ডেট সম্পূর্ণ বাদ রাখা হয়েছে
    });
    ...
    const paidWeeklyOff = records.filter(a => a.status === "Weekly Off Paid").length;
