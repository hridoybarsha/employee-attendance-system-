...
function animateValue(id, start, end, duration = 800, prefix = '') {
    ...
    const step = (timestamp) => {
        ...
        if (currentValue < 0) {
            obj.textContent = "-Tk " + Math.abs(currentValue).toLocaleString('en-IN');
        } else {
...
//  ATTENDANCE PAGE MONEY SUMMARY UPDATE FUNCTION (FIXED LOGIC)
...
function calculateEmployeePayroll(employee, month) {
    ...
    //     =   -   (  -  = - )
    const companyDue = Math.round(earnedSalary + bonus - deduction - advanceTaken);
