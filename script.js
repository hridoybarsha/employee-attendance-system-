...
function updateDashboard() {
    ...
    animateValue("presentToday", 0, todayRecords.filter(a => a.status === "Full Day" || a.status === "Full Day/Night Shift" || a.status === "Night/Full Day Shift").length);
...
/* ================= PAYROLL (FIXED TO EXACT 30 DAYS CALCULATION FOR 10,833 TARGET) ================= */
function calculateEmployeePayroll(employee, month) {
    ...
    const daysInMonth = 30; // সবসময় ৩০ দিন ধরে হিসাব হবে

    const totalWorkingDays = fullDay + nightShift + (fullDayNightShift * 2) + (nightFullDayShift * 2) + paidLeave + (halfDay * 0.5);
    
    // নিখুঁতভাবে ৩০ দিয়ে ভাগ করে ওয়ার্কিং ডে দিয়ে গুণ
